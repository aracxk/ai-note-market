...
describe("InMemoryPurchaseHistoryQueryService", () => {
	...
	it("購入履歴と記事タイトルが結合（JOIN）され、新しい順に取得できること", async () => {
		const buyerId = Result.unwrap(UserId.create("buyer-1"));
		const title1 = Result.unwrap(NoteTitle.create("Awesome TypeScript"));
		const title2 = Result.unwrap(NoteTitle.create("Next.js Mastery"));
		...
		const price500 = Result.unwrap(Price.create(500));
		const price1000 = Result.unwrap(Price.create(1000));
		...
		const purchase1 = Purchase.reconstruct({
			id: Result.unwrap(PurchaseId.create("purchase-1")),
			noteId: Result.unwrap(NoteId.create("note-1")),
			buyerId: buyerId,
		...
		const purchase2 = Purchase.reconstruct({
			id: Result.unwrap(PurchaseId.create("purchase-2")),
			noteId: Result.unwrap(NoteId.create("note-2")),
			buyerId: buyerId,
	...
	it("購入履歴に紐づく記事が存在しない場合はその履歴をスキップすること", async () => {
		const buyerId = Result.unwrap(UserId.create("buyer-missing-note"));

		// 記事データはリポジトリに保存しない（存在しない状態にする）
		const purchase = Purchase.reconstruct({
			id: Result.unwrap(PurchaseId.create("purchase-missing")),
			noteId: Result.unwrap(NoteId.create("note-missing")), // 存在しない記事ID
			buyerId: buyerId,
			purchasedPrice: Result.unwrap(Price.create(500)),

...
### ⑦ スキーマの過剰な共通化の禁止（フィーチャー独立性の優先）
- ID などの頻出フィールドであっても、`shared/schemas/` 等に安易に共通化せず、各フィーチャーの `schemas/` 配下に独立して定義する。
- 共通化による不要な結合（変更の波及）を防ぎ、機能ごとの独立性・文脈に応じたエラーメッセージの柔軟性を優先する（シェアード・ナッシング原則）。

---

## 13. CI/CD と静的解析・フォーマット規約

開発体験（DX）とコード品質の両立のため、実行タイミングを厳密に分離する。

### ① ローカル環境（Commit直前）: Biome + Husky
- `git commit` 時に Husky および `lint-staged` を経由して、**変更されたファイルのみ**に対して `biome check --write` を実行する。
- 数ミリ秒で完了する超高速な静的解析と自動フォーマット（タブインデント等）のみをコミット前に行い、開発のテンポ（Flow）を決して阻害しない。
- `pre-commit` フック内でテスト（`npm test`）や型チェック（`tsc`）などの重い処理を実行することは厳禁とする。

### ② クラウド環境（Push後）: GitHub Actions
- `git push` をトリガーとして、GitHub Actions 上で `npm run typecheck` および `npm run test` を実行する。
- 実行に数秒〜数十秒かかる重い検証処理はクラウド側に完全にオフロードし、メインブランチの品質を担保する「最後の砦」として機能させる。
